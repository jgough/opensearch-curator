...
See the {ref}/snapshots-restore-snapshot.html#change-index-settings-during-restore[official OpenSearch Documentation].
...
NOTE: https://www.elastic.co/guide/en/opensearch/reference/current/indices-shrink-index.html#_shrinking_an_index[Only `settings` and `aliases` are acceptable] when used in <<shrink,shrink>>.
...
https://www.elastic.co/guide/en/opensearch/reference/5.4/index-modules.html#_static_index_settings[static]
...
NOTE: https://www.elastic.co/guide/en/opensearch/reference/5.4/index-modules.html#dynamic-index-settings[Dynamic]
...
The value of this setting determines whether OpenSearch should include index
...
The value of this setting determines whether OpenSearch should include the
...
rollover.  Sizes must use OpenSearch approved
...
or use OpenSearch {ref}/date-math-index-names.html[date math]
...
In some cases you may be obliged to connect to a remote OpenSearch cluster
...
In this configuration, OpenSearch will capture whatever appears after `index`
...
YAML syntax, you recreate the body sent to OpenSearch as described in
...
by OpenSearch, and Curator is making a separate connection, it is important
to ensure that both Curator _and_ your OpenSearch cluster have access to the
remote host.

If you do not whitelist the remote cluster, you will not be able to reindex.
This can be done by adding the following line to your `opensearchpy.yml` file:
...
or by adding this flag to the command-line when starting OpenSearch:

[source,sh]
-------------
bin/opensearch -Edefault.reindex.remote.whitelist="remote_host_or_IP:9200"
-------------
...
The value of this setting must be the valid name of a node in your OpenSearch cluster,
...
Read {ref}/docs-index_.html#index-wait-for-active-shards[the OpenSearch documentation] for more information.
