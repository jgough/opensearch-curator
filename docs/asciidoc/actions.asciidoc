...
{ref}/indices-aliases.html[OpenSearch Alias API documentation].
...
configuration options, read the OpenSearch
...
* <<option_extra_settings,extra_settings>> No default value.  You can add any
    ...
    {ref}/indices-create-index.html[OpenSearch Create Index API documentation]
...
[IMPORTANT]
=======================
While OpenSearch allows for either dotted notation of index settings, such as
...
vs.
https://www.elastic.co/guide/en/opensearch/reference/5.4/index-modules.html#dynamic-index-settings[dynamic]
...
any version of OpenSearch, 1.4 and newer. Strictly speaking, the Reindex API
in OpenSearch _is_ able to reindex from older clusters, but Curator cannot be
used to facilitate this due to Curator's dependency on changes released in 1.4.

However, there is a https://github.com/elastic/opensearch/pull/23805[known bug]
with OpenSearch 5.3.0 not being able to reindex from remote clusters older
than 2.0.  The patch is available in OpenSearch 5.3.1.  Earlier versions of
OpenSearch 5.x do not suffer from this bug. This bug appeared again in
OpenSearch 5.4.0, and was fixed in 5.4.1, and hopefully will not appear in
...
For more information see the {ref}/snapshots-restore-snapshot.html#change-index-settings-during-restore[official OpenSearch Documentation].
...
{ref}/indices-rollover-index.html[OpenSearch Rollover API] to create a new
...
* <<option_max_size,max_size>> **Only supported in OpenSearch 6.1.0 and up.**
...
https://www.elastic.co/guide/en/opensearch/reference/current/indices-shrink-index.html#_shrinking_an_index[Several conditions need to be met]
